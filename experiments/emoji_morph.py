...
def train_emoji_sequence(filename_sequence,model_filename,downsample=2,LOSS_FUNC=None,OPTIMIZER="Nadam",KERNEL_TYPE="ID_DIFF_LAP"):
	data = load_emoji_sequence(filename_sequence,downsample=downsample)
	print(data)
	ca = NCA(N_CHANNELS,ACTIVATION="relu",REGULARIZER=0.1,LAYERS=2,KERNEL_TYPE=KERNEL_TYPE,PADDING="zero")
	...
	print(ca)
	trainer.train_sequence(2000,32,LOSS_FUNC=LOSS_FUNC,OPTIMIZER=OPTIMIZER,LEARN_RATE=2e-3)
...
def save_emoji_trajectory():
	...
	ca = load_wrapper("model_exploration/Nadam_euclidean_emoji_3_layer_ID_DIFF_LAP_relu_v1")
	#ca.ACTIVATION = "relu"
	trajectory = ca.run(data[0],I)
	W = trajectory.shape[2]
	trajectory_hidden = np.zeros((trajectory.shape[0],
							      trajectory.shape[1],
								  2*trajectory.shape[2],
								  2*trajectory.shape[3],
								  4))
	trajectory_hidden[:,:,:W,:W] = trajectory[...,:4]
	trajectory_hidden[:,:,:W,W:] = (1+np.tanh(trajectory[...,4:8]))/2.0
	trajectory_hidden[:,:,W:,:W] = (1+np.tanh(trajectory[...,8:12]))/2.0
	trajectory_hidden[:,:,W:,W:] = (1+np.tanh(trajectory[...,12:16]))/2.0
	my_animate(trajectory[:,0,...,:4])
	my_animate(trajectory_hidden[:,0])
	vis = NCA_Visualiser([ca])
	#vis.save_image_sequence_RGBA(trajectory[...,:4],"figures/alien_microbe_rooster_stable/")
	vis.save_image_sequence_RGBA(trajectory_hidden,"figures/alien_microbe_rooster_stable/animation/relu_3layer/")
...
def main():
	...
	if index==13:
		train_emoji_sequence_masked(["alien_monster.png","rooster_1f413.png","rooster_1f413.png"],
							 "emoji_masked_debug")
	if index==14:
		train_emoji_sequence(["alien_monster.png","rooster_1f413.png","alien_monster.png","rooster_1f413.png","alien_monster.png"],
					         "emoji_alien_monster_rooster_oscillator")
	if index==15:
		train_emoji_sequence(["alien_monster.png","rooster_1f413.png"],"emoji_vgg_test",LOSS_FUNC=loss_vgg)
