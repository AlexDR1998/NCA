...
def compare_NCA_PDE(LOSS_FUNC_STRING="spectral_euclidean",SAMPLING=8):
    ...
    ca = load_wrapper("model_exploration/Nadam_spectral_coral_2_layer_ID_LAP_relu_v1")
	#ca.PADDING="periodic"
    #ca.FIRE_RATE = 0.5
    # V = NCA_Visualiser([ca])
    ...
    my_animate(trajectory_true[::R,0,...,0])
    my_animate(trajectory[::R,0,...,0])
    #my_animate(trajectory_2[::R,0,...,0])
    tra_diff = np.abs(trajectory[::R,...,0]-trajectory_true[::R,...,0])
    my_animate(tra_diff[:,0])
    #my_animate(np.abs(trajectory[::R,0,...,0]-trajectory_2[::R,0,...,0]))
    ...
    vis.save_image_sequence(trajectory_true[::R,...,0],"figures/readif/coral/relu/FFT/PDE/")
    vis.save_image_sequence(trajectory[::R,...,0],"figures/readif/coral/relu/FFT/NCA/")
    vis.save_image_sequence(tra_diff,"figures/readif/coral/relu/FFT/Error/")
compare_NCA_PDE()
